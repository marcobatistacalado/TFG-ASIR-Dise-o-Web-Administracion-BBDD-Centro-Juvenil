...
<a href="../index.php"><img src="../main/img/logosls2.png"></a>
    <h2>Registro Campamento La Adrada</h2>
...
if (isset($_POST['enviar'])) {
    ...
    if (!isset($_POST['musical']) && !isset($_POST['transferencia']) && !isset($_POST['cod_form']) && !isset($_POST['participante'])&& !isset($_POST['medica'])) {
                ...
                if ($error_DNI_menor == "" && $error_reserva == "") {
                    ...
                    //Realizar la reserva 
                        try {
                            ...
                            if (mysqli_query($conn, $sql)) {
                                $compra = "<br>Registro realizado para el campamento.";
                                incrementarPlazasOcupadas($cod_form);
                            }else{
...
if($mostrarform == true){
?>
    <form method="post" action="adrada.php" enctype="multipart/form-data">
        <br><br>
        <?php
            $tipo = hijos_usuario($usuario);
            if ($tipo == 1) {
...
<br>    <label for="medica">PDF de ficha médica:</label>
        <input type="file" name="medica" id="medica" accept="application/pdf" required>
        <br><br>
        <label for="participante">Imagen de cara del participante tipo DNI:</label>
        <input type="file" name="participante" id="participante" accept="image/jpeg, image/png" required>
        <br><br>
        ...
        <input type="hidden" name="cod_form" value="ADRADA_23">
